...
SNSから受信したイベントを処理し、
Discord WebhookへPOSTするPythonコードを実行する。
...
- CloudFrontでは静的コンテンツのみキャッシュし、APIはBehaviorによってキャッシュしない設計とした。
...
CloudFront導入時には504 Gateway Timeoutが発生した。CloudFront→ALB間がHTTPSになっており、ALBはHTTPのみだったため、Origin Protocol PolicyをHTTPへ変更し解決した。
...
- 監視・ログ分析機能のさらなる強化
- Terraform.Amazon RDS.Cloud Watch Logs.AWS WAF.ECS.Blue/Green Deploymentの導入
